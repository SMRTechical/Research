...
class CurrentMedicationsSymptomatic extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
...
if (result) {
    return (
        ...
        <i className="fa fa-trash ml-5 text-danger" onClick={() => this.handleChange()} aria-hidden="true"></i>
        </div>
        }
        </span>
    )
    ...
    return (
        <div>
                            ...
                            {!this.props.isComplete ?
                                ...
                                <i className="fa fa-plus-circle text-primary fa-pointer mr-10" aria-hidden="true" onClick={() => this.handleToggle()}></i> :
...
{ this.props.treatmentDoseOpen &&
                        <div className="block current-medications-dose-container no-pad">
                         <div className="block-content-no-border">
                            <div className="current-medications-dose"><i className="mr-5" aria-hidden="true">-</i>
                                <span  onClick={() => this.handleChange(this.props.doses[0].controlValueId)}>{this.props.doses[0].controlValue}</span>
                            </div>
                            <div className="current-medications-dose"><i className="mr-5" aria-hidden="true">-</i>
                                <span  onClick={() => this.handleChange(this.props.doses[1].controlValueId)}>{this.props.doses[1].controlValue}</span>
                            </div>
                           
                            {/* {this.props.selectedDose &&
                            <div className="current-medications-dose-remove" >
                            <i className="fa fa-trash mr-5 text-danger" aria-hidden="true"></i>
                                <span className="text-danger" onClick={() => this.handleChange()}>{'Delete'}</span></div>
} */}
                        </div>
 ...
 export default CurrentMedicationsSymptomatic;
